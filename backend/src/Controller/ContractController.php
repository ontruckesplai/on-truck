...
class ContractController extends AbstractController
{
    private function serializeContract(Contract $c): array
    {
        return [
            ...
            'asignaciones' => array_map(function ($a) {
                // Use entity serialization for consistency
                return $a->jsonSerialize();
            }, $c->getAsignaciones()->toArray()),
        ];
    }
    ...
    public function index(EntityManagerInterface $em): JsonResponse
    {
        ...
        $data = array_map([$this, 'serializeContract'], $contracts);
    ...
    public function create(Request $request, EntityManagerInterface $em): JsonResponse
    {
        try {
            ...
            if (!empty($data['deadline'])) {
                try {
                    $contract->setDeadline(new \DateTime($data['deadline']));
                     ...
                     $contract->setDeadline(new \DateTime('+7 days'));
                }
            } else {
            ...
            return $this->json($this->serializeContract($contract), 201);
        } catch (\Exception $e) {
